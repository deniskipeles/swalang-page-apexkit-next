...
import { ChatMessage } from './ContributionChatbot';
...
const Chatbot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      // Use ApexKit AI Action
      const res = await apex.ai.run('community-helper', {
        variables: { query: userMsg }
      });

      setMessages(prev => [...prev, { role: 'model', content: res.result }]);
    } catch (e) {
      ...
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-2xl ${msg.role === 'user'
