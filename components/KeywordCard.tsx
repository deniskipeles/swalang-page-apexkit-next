...
interface KeywordCardProps {
  keyword: Keyword;
  currentUser: string;
  isDetailed?: boolean; // Controls whether to show all suggestions and the contribution form
...
const KeywordCard: React.FC<KeywordCardProps> = ({
  ...
  // Determine the primary suggestion to display on the summary card
  const topSuggestion = useMemo(() => {
    if (keyword.standardizedSuggestionId) {
      return keyword.suggestions.find(s => s.id === keyword.standardizedSuggestionId);
    }
    return sortedSuggestions[0];
  }, [keyword.suggestions, sortedSuggestions, keyword.standardizedSuggestionId]);
        ...
        {isDetailed ? (
          sortedSuggestions.length > 0 ? (
            sortedSuggestions.map((suggestion, index) => (
              <SuggestionCard
                ...
                onVote={(voteType) => onVote && onVote(keyword.id, suggestion.id, voteType)}
                onEdit={(updatedData) => onEditSuggestion && onEditSuggestion(keyword.id, suggestion.id, updatedData)}
                onDelete={() => onDeleteSuggestion && onDeleteSuggestion(keyword.id, suggestion.id)}
                onStandardize={() => onStandardize && onStandardize(keyword.id, suggestion.id)}
              />
            ))
          ) : (
          ...
          topSuggestion ? (
            <SuggestionCard
              ...
              isTopSuggestion={!isStandardized}
              isStandardized={isStandardized}
              isKeywordStandardized={isStandardized}
              currentUser={currentUser}
              onVote={() => {}} // Voting disabled on the summary list page
              onEdit={() => {}}
              onDelete={() => {}}
              onStandardize={() => {}}
            />
          ) : (
            <p className="text-slate-500 dark:text-slate-400 text-center py-4">No suggestions proposed yet.</p>
          )
        )}
      </div>
        ...
        {isDetailed ? (
          isStandardized ? (
            ...
            <button
              ...
              className="w-full bg-cyan-600 hover:bg-cyan-700 dark:hover:bg-cyan-500 text-white font-bold py-2.5 px-4 rounded-lg transition-colors duration-300"
            >
          ...
          <Link
            href={`/collaborate/${keyword.englishTerm}`}
            className="w-full text-center block bg-cyan-600 hover:bg-cyan-700 dark:hover:bg-cyan-500 text-white font-bold py-2.5 px-4 rounded-lg transition-colors duration-300"
          >
            Suggest a Term or View All ({keyword.suggestions.length})
          </Link>
        )}
      </div>
    </div>
