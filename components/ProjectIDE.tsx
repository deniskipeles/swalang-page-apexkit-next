...
// Rebuilds nested tree from a flat array of files { path, content }
...
const ProjectIDE: React.FC<ProjectIDEProps> = ({ projectId, strategy, initialTree, initialFiles, currentVersion }) => {
  ...
  const [fileSystem, setFileSystem] = useState<FileSystemNode[]>(() => {
    ...
    if (initialFiles) return buildTreeFromSplitList(initialFiles);
    return [];
  });
  ...
  const [fileContents, setFileContents] = useState<Record<string, string>>(() => {
    ...
    } else if (strategy === 'fat' && initialTree) {
  ...
  const [sseClient, setSseClient] = useState<ApexKitRealtimeSSEClient | null>(null);
  ...
  // Ensure readonly forces navigation mode
  useEffect(() => {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    const filePath = findPathById(fileSystem, file.id) || file.id;
  ...
  }, [isEditingMode, isReadOnly, fileSystem, currentVersion, projectId, router]);
  ...
  const handleSaveProject = useCallback(async () => {
    ...
    try {
      // We don't need buildFileTreeWithContent for saving anymore.
      // We just need the flat list of files with their current content.
      const filesToSave = flattenFilesForApi(fileSystem, fileContents);

      // Call updated API function
      const res = await saveProject(projectId, filesToSave);

      setDirtyFileIds(new Set());
      setConsoleLogs(prev => [...prev, `Snapshot saved! (Version ID: ${res.version_id})`]);
    } catch (e: any) {
  ...
  const handleRunCode = useCallback(async () => {
    ...
    // We append the channel and event params
    const sseUrl = `${apex.baseUrl}/api/v1/sse?channel=${channelId}&event=stdout`;

    // 2. Create EventSource explicitly WITHOUT credentials
    // This allows the browser to accept the '*' origin wildcard from the server
    const source = new EventSource(sseUrl, { withCredentials: false });

    source.onmessage = (event) => {
      try {
        // SSE data comes in as a JSON string
    ...
    source.onerror = (err) => {
      ...
      // source.close(); // Optional: close on error
    };
  ...
  const handleCreateNode = useCallback((type: 'file' | 'folder') => {
    ...
    const newNode: FileSystemNode = type === 'file' ? { id: newId, name: 'untitled.sw', type: 'file', content: '' } : { id: newId, name: 'NewFolder', type: 'folder', children: [] };
          ...
          <div className="flex-1 flex flex-col overflow-y-auto">
            <div className="flex-grow flex">
              <div className={`h-full ${isPreviewVisible && activeFileId ? 'w-1/2' : 'w-full'}`}>
                <Editor fileName={activeFile?.name || ''} content={activeFileId ? fileContents[activeFileId] ?? null : null} onContentChange={handleContentChange} readOnly={isReadOnly || !isEditingMode} />
              </div>
